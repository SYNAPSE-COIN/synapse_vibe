# Build an instruction-style dataset from an existing image set created by gen_txt2img_imgs.py
# Optionally apply learned compression using the in-repo estimators
# --------------------------------------------------------------------------------
...
# Sentinel byte pattern to mark modality boundaries
MAGIC_BYTES = b'\xF0\x9F\x8C\x88'  # rainbow emoji as a delimiter
...
USER_PROMPTS = [
    "Could you make an image of {prompt}?",
    "Please render a picture featuring {prompt}.",
    "Generate a visual of {prompt}.",
    "Produce an image that depicts {prompt}.",
    "I'm interested in {prompt}; create an image for it.",
    "txt2img({prompt})",
    "txt2{format}({prompt})",
]

ASSISTANT_PROMPTS = [
    "Sure—I'll create an image based on your request.",
    "Absolutely, generating that image now.",
    "On it—I'll render an image for this prompt.",
    "Certainly—preparing the image.",
    "Ready—I'll generate an image for you.",
]
...
def overfitting_guided_compression(in_image_path,
    ...
    for iteration in range(max_iterations):
        ...
        optimized_params = compressor.get_optimized_params(quality_range, width_range, target)
...
def create_instruct_sample(cfg_user_prompt: str,
    ...
    sample_end = (
        f"\n\nHere is the image generated from your request."
    ).encode('utf-8')
...
def generate_dataset(src_dataset: ImageDataset,
    ...
    b_use_compression = len(cfg_compression_formats) > 0 and cfg_compression_formats != ["none"]
    ...
    with Progress() as progress:
        ...
        for i, entry in enumerate(src_dataset.images):
            ...
            instruct_blob = create_instruct_sample(user_prompt, assistant_prompt, in_image_path)

            out_instruct_sample_path = out_instruct_samples / f"sample_{i:04d}.bin"
            write_instruct_sample(instruct_blob, out_instruct_sample_path)

            sample_rec = InstructSample(
                id=i,
            ...
            out_dataset.samples.append(sample_rec)
...
def main(args):
    _console.print(Panel.fit("Instruct Image Dataset Builder", style="bold magenta"))
    ...
    try:
        ...
        with _console.status("[bold green]Loading existing dataset..."):
            src_dataset = ImageDataset(**load_json(in_dataset_json.as_posix()))
            _console.log(f"[green]Loaded dataset with {len(src_dataset.images)} images")
        ...
        with _console.status("[bold green]Saving dataset..."):
            out_dataset.metadata = {
                'num_samples': len(out_dataset.samples),
                'magic_bytes': base64.b64encode(MAGIC_BYTES).decode('ascii'),
                'format_version': '1.0',
                'description': 'Instruction dataset coupling text prompts with (optionally) compressed images',
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create an instruction dataset with optional compressed images")
    parser.add_argument("--input-dataset", type=str, required=True, help="Base name of the source dataset (no .json extension)")
    parser.add_argument("--output-dir", type=str, default=const.dir_instruct_datasets.relative_to(const.project_root), help="Destination directory for results")
    parser.add_argument("--compression", type=str, nargs='+', choices=["jpeg", "png", "webp"], default=["none"], help="Compression format(s) to consider")
    parser.add_argument("--quality", type=str, default="90", help="JPEG/WebP quality (0-100, supports ranges and commas)")
    parser.add_argument("--compression-level", type=str, default="3", help="PNG compression level (0-9, supports ranges and commas)")
    parser.add_argument("--resize", type=str, help="Resize width (supports ranges and commas)")
    parser.add_argument("--min-bytes", type=int, default=0, help="Minimum size in bytes for compressed images")
    parser.add_argument("--max-bytes", type=int, default=math.inf, help="Maximum size in bytes for compressed images")
